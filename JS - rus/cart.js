...
function afiseazaCos() {
  ...
  if (cos.length === 0) {
      ...
      mesajText.textContent = "Ваша корзина пуста.";
      mesajGol.appendChild(mesajText);
      mesajText.style.color = "#ffbd49";

      const butonContinua = document.createElement("a");
      butonContinua.href = "../Pag-rus/produse.html"; 
      butonContinua.textContent = "Продолжить покупки";
...
function submitOrder() {
  ...
  if (!phoneRegex.test(phone)) {
      alert('Номер телефона должен содержать ровно 9 цифр и не содержать букв!');
      return;
  }
  ...
  alert(`Спасибо за ваш заказ, ${firstName} ${lastName}!\nОбщий: ${total} MDL.\nМы свяжемся с вами по адресу ${phone} или по электронной почте ${email}.\nАдрес доставки: ${address}`);
  window.location.href = "../Pag-rus/index.html";
