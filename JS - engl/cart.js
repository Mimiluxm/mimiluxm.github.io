...
function afiseazaCos() {
  ...
  if (cos.length === 0) {
      ...
      mesajText.textContent = "Your cart is empty";
      mesajGol.appendChild(mesajText);
      mesajText.style.color = "#ffbd49";

      const butonContinua = document.createElement("a");
      butonContinua.href = "../Pages/produse.html"; 
      butonContinua.textContent = "Continue shopping";
...
function submitOrder() {
  ...
  if (!phoneRegex.test(phone)) {
      alert('The phone number must contain exactly 9 digits and not include letters!');
      return;
  }
